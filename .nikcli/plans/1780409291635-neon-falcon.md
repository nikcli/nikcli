...
| Docs                                                                                                          | 5 markdown files, including TUI plugins spec                                                                                                                                       | `specs/`, external `nikcli-ai.dev/docs`                        |
...
| #   | Item                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Pri    | Effort |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ------ |
| M1  | **Stale OAuth URL.** `src/auth/index.ts:310`, `src/account/url.ts:7`, `src/account/index.ts:29` hardcode `https://nikcli.mintlify.app/...`. Every other reference in the codebase uses `nikcli-ai.dev` (e.g. `installation/index.ts:153`, `server/server.ts:1040`, `provider/provider.ts:1078`). The `mintlify.app` host is a docs-mirror that was never the auth origin. Result: OAuth refresh for the bundled `nikcli` provider silently fails on every new login. | **P0** | S      |
...
| M5  | **Catch-all marketing-site proxy.** `src/server/server.ts:1037-1040` routes every unmatched path to `https://app.nikcli-ai.dev`. This is convenient for embedding the Web UI but means a typo in a user-typed URL silently fetches the marketing site. Make this an opt-in `NIKCLI_EMBED_WEB=1` flag with an explicit no-match 404.                                                                                                                                  | P1     | S      |
