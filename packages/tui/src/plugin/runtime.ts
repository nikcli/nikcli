...
import {
  ...
  type PluginOptions,
  type PluginSpec,
} from "@nikcli-ai/util/plugin-spec";
import {
  pluginHost,
...
async function loadExternalPlugin(
  ...
  const entry = await resolvePluginEntrypoint(spec, target, "tui").catch(
    (error) => {
  ...
  const mod = await import(version)
    .then((raw) => {
      ...
      return (
        (readV1Plugin(value, spec, "tui", "detect") as
          | TuiPluginModule
          | undefined) ?? readV2TuiPlugin(value, spec)
      );
    })
    .catch((error) => {
...
async function activateWithBudget(state: RuntimeState, plugin: PluginEntry) {
  ...
  try {
    const outcome = await Promise.race([
      activation.then(() => "done" as const),
      budget,
    ]);
    if (outcome === "timeout") {
      log.warn(
        "tui plugin is still activating; continuing startup without it",
        {
          id: plugin.id,
      ...
      void activation.catch((error) =>
        log.error("tui plugin activation failed", { id: plugin.id, error }),
      );
      return;
    }
...
async function activatePluginById(
...
async function deactivatePluginById(
  state: RuntimeState | undefined,
  id: string,
  persist: boolean,
) {
...
function pluginApi(
  ...
  const registerSlot = (
    plugin: Parameters<TuiPluginApi["slots"]["register"]>[0],
  ) => {
 ...
 */
async function swapPluginEntry(
  state: RuntimeState,
  ...
  if (!next) {
    reportReloadFailure(
      state,
      previous.id,
      `${load.spec}: reloaded module has no tui export (previous version kept)`,
    );
    return false;
  }
...
async function reloadPluginEntry(
  ...
  if (!override?.force) {
    ...
    if (freshSpecifier(plugin.load.entry, mtime) === plugin.load.version)
      return false;
  }

  let error: unknown;
  const load = await loadExternalPlugin(
    item,
    override?.meta ?? plugin.load.config_meta,
    false,
    {
  ...
  const hit = await PluginMeta.touchMany([
    { spec: load.spec, target: load.target, id: load.id },
  ])
    .then((rows) => rows?.[0])
    .catch((error) => {
  ...
  return swapPluginEntry(
    state,
    plugin,
    load,
    createMeta(load.source, load.spec, load.target, hit, load.id),
  );
}

function sameOptions(
  left: PluginOptions | undefined,
  right: PluginOptions | undefined,
) {
...
async function removePluginEntry(state: RuntimeState, plugin: PluginEntry) {
  ...
  if (state.plugins_by_id.get(plugin.id) === plugin)
...
async function reconcileConfiguredPlugins(state: RuntimeState) {
  ...
  for (const item of config.plugin ?? [])
    desired.set(pluginSpecifier(item), item);
  ...
  for (const [spec, item] of desired) {
    ...
    if (!load) {
      reportReloadFailure(
        state,
        spec,
        `${spec}: ${errorMessage(error ?? new Error("failed to load"))}`,
      );
      continue;
    }
    if (state.plugins_by_id.has(load.id)) {
      reportReloadFailure(
        state,
        spec,
        `${spec}: plugin id ${load.id} is already loaded`,
      );
      continue;
    }
...
async function reloadLocalPlugins(state: RuntimeState) {
  // No instance wrapper: every host call below takes its own directory.
  {
    ...
    for (const plugin of state.plugins.filter(
      (item) => item.load.source === "file" && item.load.item,
    )) {
...
function applyInitialPluginEnabledState(
  state: RuntimeState,
...
async function resolveExternalPlugins(
  ...
  const loaded = await Promise.all(
    list.map((item) => loadExternalPlugin(item, meta(item), false, { origin })),
  );
...
async function addExternalPluginEntries(
  state: RuntimeState,
  ready: PluginLoad[],
) {
...
function defaultPluginMeta(state: RuntimeState): PluginConfigMeta {
  return {
    ...
    source:
      ...
      path.join(state.directory, ".nikcli", "tui.json"),
  };
}
...
export namespace TuiPluginRuntime {
  ...
  export async function dispose() {
    ...
    if (Date.now() > deadline) {
      log.warn(
        "tui plugin shutdown exceeded its budget; remaining cleanups were not waited on",
        {
    ...
    try {
      {
        ...
        dbg(
          "runtime: internal plugins",
          INTERNAL_TUI_PLUGINS.map((x) => x.id).join(","),
        );
