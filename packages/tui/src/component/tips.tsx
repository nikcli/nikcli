...
const TIPS = [
  ...
  "Run {highlight}/share{/highlight} to create a public link to your conversation at nikcli-ai.dev",
  ...
  "Create {highlight}nikcli.json{/highlight} in project root for project-specific settings",
  "Place settings in {highlight}~/.config/nikcli/nikcli.json{/highlight} for global config",
  ...
  "Nikcli auto-handles OAuth for remote MCP servers requiring auth",
  "Add {highlight}.md{/highlight} files to {highlight}.nikcli/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.nikcli/agent/{/highlight} for specialized AI personas",
  "Configure per-agent permissions with {highlight}ctrl+s{/highlight} — presets and per-tool Allow / Ask / Deny",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "Nikcli auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "Nikcli uses LSP servers for intelligent code analysis",
  "Create {highlight}.ts{/highlight} files in {highlight}.nikcli/tool/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.nikcli/plugin/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Nikcli from reading sensitive files",
  "Use {highlight}nikcli run{/highlight} for non-interactive scripting",
  "Use {highlight}nikcli --continue{/highlight} to resume the last session",
  "Use {highlight}nikcli run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}nikcli serve{/highlight} for headless API access to Nikcli",
  "Use {highlight}nikcli run --attach{/highlight} to connect to a running server",
  "Run {highlight}nikcli upgrade{/highlight} to update to the latest version",
  "Run {highlight}nikcli auth list{/highlight} to see all configured providers",
  "Run {highlight}nikcli agent create{/highlight} for guided agent creation",
  ...
  "Run {highlight}nikcli debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/nikcli/nikcli{/highlight} for containerized use",
