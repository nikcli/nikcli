...
export default function SessionScreen() {
  ...
  const loadGitState = useCallback(async () => {
    ...
    try {
      setGitLoading(true);
      const gitClient = gitDirectory
        ? client.withDirectory(gitDirectory)
        : client;
  ...
  const backgroundTasks = useMemo(
    () => collectBackgroundTasks(messages),
    [messages],
  );
  const runningBackgroundTasks = useMemo(
    () => runningTasks(backgroundTasks),
    [backgroundTasks],
  );
  ...
  const cleaned = Boolean(
    detail?.info.github?.worktree.cleanedAt ?? detail?.info.worktree?.cleanedAt,
  );
  const hasCleanableWorktree = Boolean(
    detail?.info.github?.worktree ?? detail?.info.worktree,
  );
  ...
  const sessionLocation = [sessionProject, sessionOwner]
    .filter(Boolean)
    .join(" · ");
  ...
  async function cleanup() {
    ...
    try {
      ...
      const repositoryDirectory =
        ...
        detail?.info.worktree?.repositoryDirectory;
  ...
  const paletteItems = useMemo<CommandPaletteItem[]>(() => {
    ...
    const localItems: CommandPaletteItem[] = [
      ...
      },
      {
        ...
        description:
          "Todos, MCP, LSP, context, and modified files for this session.",
        section: "View",
        badge: "Local",
      ...
      */}
      <View style={{ flex: 1 }}>
        ...
        <FlashList
          ...
          getItemType={(row) =>
            row.kind === "message" ? row.message.info.role : row.kind
          }
          ...
          renderItem={({ item: row }) => {
            ...
            if (row.kind === "system")
              return <ScaffoldingRow message={row.message} />;
      ...
      <BackgroundActivitySheet
        ...
        onStop={(task: BackgroundTask) => {
          ...
          void client
            .abortSession(task.childSessionID)
        ...
        onOpenTranscript={(task: BackgroundTask) => {
          if (task.childSessionID)
            router.push(`/sessions/${task.childSessionID}`);
        }}
      />
