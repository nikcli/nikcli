...
/// ADE patch: whether the next encoder may use a hardware MFT.
///
/// Upstream always asks for hardware. A process whose hardware encoder
/// session is in a bad state then fails every new encoder with
/// MF_E_TRANSFORM_TYPE_NOT_SET until it restarts; software keeps working.
static HARDWARE_ACCELERATION: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(true);

/// Chooses hardware (default) or software encoding for encoders created from now on.
pub fn set_hardware_acceleration(enabled: bool) {
    HARDWARE_ACCELERATION.store(enabled, std::sync::atomic::Ordering::Relaxed);
}

fn hardware_acceleration() -> bool {
    HARDWARE_ACCELERATION.load(std::sync::atomic::Ordering::Relaxed)
}
