...
class WipeableTarget extends EventTarget {
  ...
  wipe() {
    for (const [type, listener, options] of added.get(this) ?? [])
...
function makeWindow(input: {
  ...
  const fromParent = (data: unknown) =>
    win.dispatchEvent(
      new BrowserMessageEvent("message", { data, source: win.parent }),
    );
  const fromPage = (data: unknown) =>
    win.dispatchEvent(
      new BrowserMessageEvent("message", { data, source: win }),
    );
  return {
    win,
    topPosted,
...
describe("frameGuard: any frame", () => {
  ...
  test("the guard cannot be taken back", () => {
    ...
    expect(() =>
      Object.defineProperty(win, "Headers", { value: Headers }),
    ).toThrow();
...
describe("frameGuard: a browser pane's frame", () => {
  ...
  test("a hello on the window starts nothing, and the page sees it as any message", () => {
    ...
    frame.win.addEventListener("message", (event: any) =>
  ...
  test("answers a ping, before and after the page wipes the window's listeners", () => {
    ...
    frame.wipeListeners();
    frame.port!.postMessage({ type: "ade-browser:ping" });
    expect(frame.heard).toEqual([
      { type: "ade-browser:pong" },
      { type: "ade-browser:pong" },
    ]);
  });
  ...
  test("the bridge hears the pane, not the page", () => {
    ...
    frame
      .shim()
      .addEventListener("message", (event: any) => heard.push(event.data));
  ...
  test("the bridge's input listeners let no made-up event through", () => {
    ...
    frame
      .shim()
      .__ADE_LISTEN__(target, "click", (event: unknown) => heard.push(event));
...
describe("FrameGate", () => {
  ...
  function asker() {
    ...
    frame.onmessage = (event: any) => {
      ...
      if (event.data.type === "ade-browser:ping")
        frame.postMessage({ type: "ade-browser:pong" });
      if (event.data.type === "ade-browser:hello")
        state.hello = event.data.secret;
    };
...
describe("openEnvelope", () => {
  ...
  test("opens only an envelope with the pane's secret", () => {
    ...
    expect(
      openEnvelope({ type: FRAME_ENVELOPE, secret, message }, secret),
    ).toEqual(message);
    expect(
      openEnvelope({ type: FRAME_ENVELOPE, secret: "guess", message }, secret),
    ).toBeUndefined();
    expect(
      openEnvelope({ type: FRAME_ENVELOPE, message }, secret),
    ).toBeUndefined();
  ...
  test("an empty or short secret opens nothing", () => {
    expect(
      openEnvelope({ type: FRAME_ENVELOPE, secret: "", message: 1 }, ""),
    ).toBeUndefined();
  });
