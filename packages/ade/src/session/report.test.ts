...
describe("readReportLine — tokens", () => {
  ...
  test("embedded in a sentence", () => {
    expect(readReportLine({}, "Session used 8400 tokens so far").tokens).toBe(8400)
  })

  test("ANSI-decorated line", () => {
    expect(readReportLine({}, "\x1b[36m↓ 5.4k tokens\x1b[0m").tokens).toBe(5400)
  })
...
describe("readReportLine — cost", () => {
  ...
  test("dollari suffix", () => {
    expect(readReportLine({}, "Totale: 0,25 dollari").costUsd).toBeCloseTo(0.25)
  })

  test("ANSI-decorated amount", () => {
    expect(readReportLine({}, "\x1b[32m$0.42\x1b[0m").costUsd).toBeCloseTo(0.42)
  })
...
describe("readReportLine — model", () => {
  test("'Using model:' declaration", () => {
    expect(readReportLine({}, "Using model: claude-opus-4-6").model).toBe("claude-opus-4-6")
  })
  ...
  test("bare key: declaration", () => {
    expect(readReportLine({}, "model: gemini-3-pro").model).toBe("gemini-3-pro")
  })
...
describe("readReportLine — activity", () => {
  ...
  test("English gerund", () => {
    expect(readReportLine({}, "Reading files…").activity).toBe("Reading files…")
  })

  test("state verb from the known list", () => {
    expect(readReportLine({}, "Cerco nel workspace…").activity).toBe("Cerco nel workspace…")
  })

  test("ASCII ellipsis works too", () => {
    expect(readReportLine({}, "Compiling project...").activity).toBe("Compiling project...")
  })

  test("ANSI-decorated spinner line", () => {
    expect(readReportLine({}, "\x1b[90mRagionando…\x1b[0m").activity).toBe("Ragionando…")
  })

  test("long line is truncated at a word boundary", () => {
    const line = "Analizzando la struttura del pacchetto per capire dove intervenire…"
...
describe("readReportLine — purezza e righe combinate", () => {
  test("status line carrying several fields at once", () => {
    const next = readReportLine({}, "↓ 5.4k tokens · $0.42 · model=claude-opus-4-6")
    expect(next).toEqual({
      tokens: 5400,
      costUsd: 0.42,
