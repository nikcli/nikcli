/**
 * Everything in @nikcli-ai/voice except the Solid components.
 *
 * Why this entry point exists:
 * Solid's JSX has no automatic runtime - it needs Solid's own compiler, which
 * only a bundler provides. So a .tsx file here cannot be loaded by a plain
 * TypeScript runtime, and the main barrel re-exports three of them. Anything
 * wanting the settings model, the parser or the engine without standing up a
 * Solid build - a unit test, a headless consumer, a CLI - imports this instead
 * and gets the same logic with none of the compiler requirement.
 *
 * Kept as an explicit mirror of index.ts rather than a wildcard re-export: the
 * Effect service tags are deliberately named after the interfaces they provide
 * (Transcriber, Speaker, MicCapture), so export * collides on all three.
 */
