...
export interface InspectedElement {
  ...
  /**
   * The part of the page the element sits in, as a person would name it:
   * hero, header, nav, footer, pricing… With the heading it carries, when
   * there is one: `hero «Build faster»`. Absent when nothing around it reads
   * as a section.
   */
  section?: { name: string; label: string; selector: string }
  /** The element is itself a section; its label, as in `section`. */
  ownSection?: string
  /** Only text inside: its text can be edited in place. */
  textOnly?: boolean
}
...
export type BridgeMessage =
  ...
  | { type: "visual-editor:select-section"; selector: string }
  /** Host → page: replace the text of a text-only element. */
  | { type: "visual-editor:apply-text"; selector: string; text: string }
  /** Page → host: an edit made in place, with the value it replaced. */
  | { type: "visual-editor:edit-applied"; selector: string; property: string; before: string; after: string }
