...
import { relayStartupTerminal, startupTerminalMode } from "./tui-startup-terminal"
...
if (terminalMode === "attached-terminal" && !reportPath) {
  throw new Error("TERMINAL_RELAY=1 requires REPORT_PATH; stdout belongs to the terminal")
}
...
const COLS = envInt("COLS", terminalMode === "attached-terminal" ? process.stdout.columns : 100)
const ROWS = envInt("ROWS", terminalMode === "attached-terminal" ? process.stdout.rows : 30)
...
async function once(home: string): Promise<Attempt> {
  ...
  live.add(pty)
  if (terminalMode === "attached-terminal") restoreTerminal = relayStartupTerminal(pty)
