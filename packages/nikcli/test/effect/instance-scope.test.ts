...
const testHome = await fs.mkdtemp(
  path.join(os.tmpdir(), "nikcli-effect-instance-home-"),
);
...
async function makeProjectDir() {
  const dir = await fs.mkdtemp(
    path.join(os.tmpdir(), "nikcli-effect-instance-project-"),
  );
    ...
    const seen = await Effect.runPromise(
      InstanceScope.with(
        ...
        Effect.gen(function* () {
          ...
          const viaPromise = yield* Effect.promise(
            async () => Instance.directory,
          );
...
describe("withInstanceAsync with an init", () => {
  it("runs init once per directory and provides the instance to the body", async () => {
    ...
    const first = await withInstanceAsync(
      { directory, init },
      async () => Instance.directory,
    );
    const second = await withInstanceAsync(
      { directory, init },
      async () => Instance.directory,
    );
  ...
  it("rejects with the body's own error, not a re-wrapped one", async () => {
    ...
    const caught = await withInstanceAsync(
      { directory, init: async () => {} },
      async () => {
        throw thrown;
      },
    ).catch((error) => error);
  ...
  it("provides WorkspaceRef alongside init", async () => {
    const directory = await makeProjectDir();
    const seen = await withInstanceAsync(
      { directory, workspaceID: "wrk_init", init: async () => {} },
      async () =>
        Effect.runPromise(
          Effect.gen(function* () {
            return yield* WorkspaceRef;
