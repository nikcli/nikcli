...
describe("EOT-02 multi-instance teardown", () => {
  it("two scopes on one directory settle independently and each bumps counters", async () => {
    await withTempHome(async (directory) => {
      const a = Effect.runPromise(InstanceScope.with({ directory }, Effect.succeed("a")))
      const b = Effect.runPromise(InstanceScope.with({ directory }, Effect.succeed("b")))
    ...
    it(`waits for owned finalizers without cancelling the peer (${sameDirectory ? "shared" : "distinct"} directory)`, async () => {
      await withTempHome(async (home) => {
        ...
        const start = (owner: string, directory: string) =>
          Effect.runFork(
            InstanceScope.with(
              ...
              Effect.scoped(
                Effect.gen(function* () {
                  yield* Effect.acquireRelease(Effect.succeed(owner), (resource) =>
                    Effect.gen(function* () {
        ...
        try {
          ...
          finishPeer.resolve()
          expect(await Effect.runPromise(Fiber.await(b))).toEqual(Exit.succeed("b"))
          ...
          expect(after["scope.interrupted"] - before["scope.interrupted"]).toBe(1)
        } finally {
          release.resolve()
          finishPeer.resolve()
          await Promise.all([Effect.runPromise(Fiber.interrupt(a)), Effect.runPromise(Fiber.interrupt(b))])
        }
        expect(released).toEqual(["b", "a"])
      })
    })
