...
describe("startup terminal evidence", () => {
  test("default remains a non-answering PTY even when attached", () => {
    expect(startupTerminalMode(undefined, false, false)).toBe(
      "non-answering-pty",
    );
  ...
  test("relay refuses headless or redirected streams", () => {
    for (const [input, output] of [
      ...
      expect(() => startupTerminalMode("1", input, output)).toThrow(
        "requires attached terminal",
      );
    }
    expect(startupTerminalMode("1", true, true)).toBe("attached-terminal");
    expect(() => startupTerminalMode("ghostty", true, true)).toThrow(
      "must be 0 or 1",
    );
  });
  ...
  test("relay carries output and replies through real PTYs and restores input ownership", async () => {
    const module = new URL(
      "../../script/tui-startup-terminal.ts",
      import.meta.url,
    ).pathname;
    const ptyModule = new URL("../../../util/src/pty.ts", import.meta.url)
      .pathname;
    ...
    try {
      const exit = await new Promise<number>((resolve, reject) => {
        timer = setTimeout(
          () => reject(new Error(`relay did not finish: ${output}`)),
          5000,
        );
        outer.onData((data) => {
          output += data;
